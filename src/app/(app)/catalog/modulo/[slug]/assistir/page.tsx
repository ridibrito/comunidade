...
export default function AssistirModuloPage({ params }: PageProps) {
  ...
  useEffect(() => {
    if (!supabase) return;
    (async () => {
      const { data: mod } = await supabase.from("modules").select("id, title").eq("slug", params.slug).maybeSingle();
      if (!mod?.id) return;
      setModuleId(mod.id);
  ...
  return (
    <Container>
      <Section>
        ...
        <div className="mt-4">
          <Link href={`/catalog/modulo/${params.slug}`} className="text-sm underline opacity-80 hover:opacity-100">Voltar para lista de aulas</Link>
        </div>
      </Section>
    </Container>
  );
