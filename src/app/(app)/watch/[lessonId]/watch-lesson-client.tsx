...
export default function WatchLessonClient({ lessonId }: { lessonId: string }) {
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_360px] items-start">
          ...
          <aside className="rounded-xl border border-[var(--border)] p-4">
            ...
            <div className="space-y-1">
              {lessons.map((l) => (
                <Link key={l.id} href={`/watch/${l.id}`} className={`block px-3 py-2 rounded-lg hover:bg-[var(--hover)] ${l.id===lessonId?"bg-[var(--hover)]":""}`}>{l.title}</Link>
              ))}
